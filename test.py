...
for i in range(plant_array_side_length):
    ...
    for j in range(plant_array_side_length):
        if ( i * plant_array_side_length + j < number_of_plants ):
            ...
            plant_row[j].xPos = ((i - (plant_array_side_length / 4)) * offset)
            plant_row[j].zPos = ((j - (plant_array_side_length / 4)) * offset)
...
def update(frame):
    ...
    ax.set_xlim(-axis_length/2.5, axis_length/2.5)
    ax.set_ylim(-axis_length/2.5, axis_length/2.5)
    ax.set_zlim(0, (axis_length/2.5)*2)
    ax.axis('off')
    month = i-12*((i-1)//12)
...
plantAnimation = animation.FuncAnimation(fig, update, frames=range(1, generations + 1), interval=100, repeat=False)
plt.show()